...
function startGame() {
    ...
    .then(response => {
        console.log("Respuesta completa:", response); // Log para ver la respuesta
        if (!response.ok) {
    ...
    .then(data => {
        ...
        document.getElementById('question').innerText = data.question || "No se recibió ninguna pregunta";
